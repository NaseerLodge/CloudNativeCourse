...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as soc:
    ...
    while True:
        ...
        with conn:
            ...
            # Receive an integer and a string from client
            data = conn.recv(1024)
            a = struct.unpack('i', data[:4])[0]
            s = data[4:].decode('utf-8')
            k = str(s)
            print(f'Received {a} and {s} from client')
            # Calculate sum using addition function
            result = recommend_by_cosine(k,a)
            print(result)
            n = len(result)
            ...
            for s in result:
                ...
                conn.sendall(struct.pack('i', len(s_data)))
                conn.sendall(s_data)
            print(f'Sent {n} strings to client')
